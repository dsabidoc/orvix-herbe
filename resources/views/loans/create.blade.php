    ...
    <form class="rounded-lg border border-slate-200 bg-white p-5 shadow-sm" method="POST" action="{{ route('loans.store') }}" enctype="multipart/form-data">
        ...
        <div class="grid gap-6 xl:grid-cols-3">
            ...
            <section class="space-y-4">
                ...
                <select class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" name="calculation_method">
                ...
                <div>
                    <input class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" name="administration_fee" placeholder="Cobranza / Gtos Admon por pago" type="number" step="0.01" value="{{ old('administration_fee', '0') }}">
                </div>
                <input class="w-full rounded-md border border-slate-300 px-3 py-2 text-sm" name="first_payment_date" type="date" value="{{ old('first_payment_date', now('America/Merida')->toDateString()) }}">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <select class="rounded-md border border-slate-300 px-3 py-2 text-sm" name="term_months" required>
                ...
                <div class="grid gap-2 sm:grid-cols-2">
                    <button class="rounded-md bg-slate-950 px-4 py-2 text-sm font-bold text-white" formaction="{{ route('loans.store') }}">Crear regular</button>
                    <button class="rounded-md bg-[#0d9488] px-4 py-2 text-sm font-bold text-white" formaction="{{ route('loans.quote-rounded') }}">Calcular con redondeo</button>
                </div>
            </section>
        </div>
    </form>
