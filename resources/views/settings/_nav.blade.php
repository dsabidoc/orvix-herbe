<nav class="mb-5 flex flex-wrap gap-2">
    ...
    <a class="{{ request()->routeIs('settings.loan-terms') ? 'bg-[#0d9488] text-white' : 'border border-slate-300 bg-white text-slate-700' }} rounded-md px-4 py-2 text-sm font-bold" href="{{ route('settings.loan-terms') }}">Plazos</a>
    <a class="{{ request()->routeIs('settings.client-merge') ? 'bg-[#0d9488] text-white' : 'border border-slate-300 bg-white text-slate-700' }} rounded-md px-4 py-2 text-sm font-bold" href="{{ route('settings.client-merge') }}">Unificar clientes</a>
</nav>
